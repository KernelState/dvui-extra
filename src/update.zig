...
test "Update Threads" {
    ...
    const UT = UpdateThread(TestMsg);
    var state = State{};
    const ut = UT{
        .self = &state,
        .update_fn = struct {
            pub fn wrapper(self_: *anyopaque, msg: TestMsg) !void {
                const self: *State = @ptrCast(@alignCast(self_));
                switch (msg) {
                    .add => |x| self.num + x,
                }
            }
    ...
    try ut.run();
    try ut.send(std.testing.allocator, .{ .add = 12 });
    try ut.send(std.testing.allocator, .{ .add = 18 });
    ut.stop();
    std.testing.assert(state.num == 30);
}
